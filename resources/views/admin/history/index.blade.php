...
@section('content')
<div class="container mx-auto w-100 px-4 md:px-24 py-8 animate__animated animate__fadeIn">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="searchInput" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" placeholder="Nama, barang, atau mapel...">
                    </div>
                </div>
                <div>
                    <label for="filterTanggal" class="block text-sm font-medium text-gray-700 mb-1">Filter Tanggal</label>
                    <input type="date" id="filterTanggal" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
                    <label for="filterStatus" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select id="filterStatus" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div class="md:col-span-3 flex justify-end">
                    <button id="resetFilter" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md transition duration-200 flex items-center">
            ...
            <div class="mt-4 pt-4 border-t border-gray-100">
                <div class="flex items-center justify-between text-sm">
                    ...
                    <span class="font-medium text-blue-600">
                        {{ count(array_filter($pengembalianHistory, fn($entry) => 
                            \Carbon\Carbon::parse($entry['tanggal_pengembalian'])->isCurrentWeek()
                        )) }}
                    </span>
                </div>
            </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="font-medium text-purple-600">
                            {{ count(array_unique(array_column(
                                array_filter($pengembalianHistory, fn($entry) => 
                                    \Carbon\Carbon::parse($entry['tanggal_pengembalian'])->isCurrentMonth()
                                ), 'name'))) 
                            }}
                        </span>
    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        <div class="flex justify-between items-center mb-8">
            ...
            <div class="flex items-center space-x-2">
                <button id="toggleView" class="px-4 py-2 bg-indigo-100 text-indigo-700 hover:bg-indigo-200 rounded-lg transition duration-200 flex items-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7"/>
                    </svg>
                    Toggle View
                </button>
                <button id="exportData" class="px-4 py-2 bg-green-100 text-green-700 hover:bg-green-200 rounded-lg transition duration-200 flex items-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="tableView" class="overflow-hidden rounded-xl border border-gray-200">
            <table class="min-w-full table-auto border-collapse">
                ...
                <tbody class="divide-y divide-gray-200">
                    @php
                        $historyData = array_merge($pengembalianHistory, Session::get('history_approvals', []));
                    @endphp
                    
                    @forelse ($historyData as $index => $entry)
                        <tr class="hover:bg-gray-50 transition duration-150 data-row" 
                            ...
                            <td class="px-6 py-4 text-center">
                                <button class="view-details text-blue-600 hover:text-blue-800 mr-2" data-id="{{ $index }}">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </button>
                                <button class="print-row text-gray-600 hover:text-gray-800">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @forelse ($historyData as $index => $entry)
                <div class="bg-white rounded-lg shadow border border-gray-100 p-4 data-card"
                    ...
                    <div class="flex justify-end space-x-2 mt-2 pt-2 border-t border-gray-100">
                        <button class="view-details text-blue-600 hover:text-blue-800 p-1" data-id="{{ $index }}">
        ...
        <div class="mt-6 flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Menampilkan <span class="font-medium">{{ count($historyData) }}</span> dari <span class="font-medium">{{ count($historyData) }}</span> data
            </div>
