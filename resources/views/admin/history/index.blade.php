...
<div class="w-100 mx-16 mt-6 animate__animated animate__fadeIn">
    ...
    <div class="mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        @php
            $currentMonthCount = $pengembalianHistory->filter(fn($entry) => \Carbon\Carbon::parse($entry->tanggal_pengembalian)->isCurrentMonth())->count();
            $currentWeekCount = $pengembalianHistory->filter(fn($entry) => \Carbon\Carbon::parse($entry->tanggal_pengembalian)->isCurrentWeek())->count();
            $uniqueUsersCount = $pengembalianHistory->pluck('name')->unique()->count();
            $currentMonthUsersCount = $pengembalianHistory->filter(fn($entry) => \Carbon\Carbon::parse($entry->tanggal_pengembalian)->isCurrentMonth())->pluck('name')->unique()->count();
        @endphp
        ...
        <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all p-6 border-l-4 border-blue-500 overflow-hidden relative group">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ $currentMonthCount }}</p>
                </div>
            </div>
            <div class="absolute -right-2 -bottom-3 opacity-5 group-hover:opacity-10 transition-opacity">
        ...
        <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all p-6 border-l-4 border-purple-500 overflow-hidden relative group">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ $uniqueUsersCount }}</p>
                </div>
            </div>
            <div class="absolute -right-2 -bottom-3 opacity-5 group-hover:opacity-10 transition-opacity">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-green-500 to-green-600 px-8 py-6 flex justify-between items-center">
            ...
            <a href="{{ route('pengembalian.export-csv') }}" class="inline-flex justify-center w-32 rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ">
        ...
        <div class="p-6 border-b border-gray-100">
            <form action="{{ route('pengembalian.history') }}" method="GET" class="flex flex-col md:flex-row flex-wrap gap-4 items-start md:items-center">
                <div class="relative flex-1 min-w-[250px]">
                    ...
                    <input type="text" 
                           name="search"
                           placeholder="Cari nama, barang, atau mapel..." 
                ...
                <div class="flex flex-wrap gap-3 items-center">
                    ...
                    <a href="{{ route('pengembalian.reset-filter') }}" class="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition flex items-center justify-center min-w-[80px] font-medium">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full table-auto border-collapse">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        <th class="px-6 py-4 text-left font-semibold text-gray-600 uppercase text-xs tracking-wider">Nama User</th>
                ...
                <tbody class="divide-y divide-gray-100" id="tableBody">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors animate__animated animate__fadeIn" data-tanggal="{{ $entry->tanggal_pengembalian }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ $entry->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            <div class="flex items-center">
                                <svg class="h-4 w-4 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                </svg>
                                {{ $entry->tanggal_pengembalian ? \Carbon\Carbon::parse($entry->tanggal_pengembalian)->format('d M Y') : '-' }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm">
                            @if($entry->status == 'Rejected')
                                <button 
                                    type="button" 
                                    class="px-2.5 py-1 bg-red-50 text-red-700 rounded-full text-xs font-medium flex items-center w-fit cursor-pointer hover:bg-red-100 transition"
                                    onclick="openAlasanModal('{{ $entry->id }}', '{{ htmlspecialchars($entry->alasan ?? 'Tidak ada alasan') }}')"
                                >
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center animate__animated animate__fadeIn">
                                ...
                                <p class="text-xl font-medium text-gray-800 mb-1">Tidak ada data riwayat pengembalian</p>
                                <p class="text-gray-500">Data riwayat pengembalian akan muncul di sini</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const filterForm = document.querySelector('form[action="{{ route("pengembalian.history") }}"]');
        if (filterForm) {
